...
def match(stillGoing, npc, skill, ndx, logFile=None):
    if ndx == len(stillGoing)-1:
        ## I got a "by"
        if logFile != None:
            logFile.write('\t' + npc[stillGoing[ndx]].ID.name + ' got a by\n')
        return stillGoing[ndx], None
    else:
        pc1 = stillGoing[ndx]
        pc2 = stillGoing[ndx + 1]
        print npc[pc1].ID.name, ' vs ', npc[pc2].ID.name
        result = npc[pc1].fight(npc[pc2], skill, logFile)
        if result:
            return pc1, pc2
        else:
            return pc2, pc1

def boysTourneyEvent(num_npc, skill, pcList=[], log=False):
    logFile = None
    if log:
        os.chdir(r'C:\Users\caHt\Documents\MyPython\tournament')
        logFile = open("logFile.txt", 'w')
        logFile.write(skill+"\nNumber contestants: {0}\n".format(num_npc))
    npc = pcList[:] # make a copy
    for i in range(num_npc - len(pcList)):
        thisPC = pc.Character()
        thisPC.addSkill(skill) # default rank is boy
        npc.append(thisPC)
    ...
    for round, victor in enumerate(victors):
        print round+1
        for v in victor:
            print npc[v].ID.name, npc[v].ability[skill]
    if log:
        logFile.close()
    return victors
